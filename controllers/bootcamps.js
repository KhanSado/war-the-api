...
exports.getBootcamp = asyncHandler(async(req, res, next) => {
    ...
    if(!bootcamp){
        return next(new ErrorResponse(`Bootcamp not found with id of ${req.params.id}`, 403))
...
exports.bootcampPhotoUpload = asyncHandler(async(req, res, next) => {
    ...
    if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(
            new ErrorResponse(`User ${req.params.id} is not authorized to upload photo this bootcamp`, 404)
        )
    }
